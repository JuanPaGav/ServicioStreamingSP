...
{
    if (get_episodios().size()==0){ // Checa si la serie no tiene episodios
    ...
    for (int i= 1; i <= get_temporadas(); i++) // Ciclo realizado por de temporada de la serie
    {
        ...
        for (Episodio& ep : episodios) { // Se recorre el vector de referencias a episodios
            if (ep.get_temporada() == i){ // Checa si el episodio pertenece a la temporada del ciclo
...
void Serie::mostrar_info() const
{
    ...
    cout << "------------- Episodios ----------------" << endl;
    for (const auto& ep : episodios) { // Recorre el vector de referencias a episodios y los imprime
        cout << "----------------" << endl;
        ep.mostrar_info();
    }
    cout << "****************************************\n" << endl;
